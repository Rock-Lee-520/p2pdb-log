...
func (tc TestCase) Multihash() (multi.Multihash, error) {
	...
	nb := append(pre[:n], ob...)
	return multi.Cast(nb)
}

func TestEncode(t *testing.T) {

	//test one
	var str = "Decentralize everything!!!Decentralize everything!!!Decentralize everything!!!Decentralize everything!!!Decentralize everything!!!Decentralize everything!!!Decentralize everything!!!"
	var Bytes = []byte(str)
	// obStr, err := HashEncode(str)
	// if err != nil {
	// 	t.Error(err)
	// }
	// debug.Dump(obStr)
	//{"2c26b46b", 0x12, "sha2-256"},
	encC, err := HashEncode(Bytes, 0x12)
	if err != nil {
		t.Error(err)
	}
	debug.Dump(hex.EncodeToString(encC))
	//test more
	for _, tc := range testCases {
		...
		encC, err := HashEncode(ob, tc.code)
		...
		encN, err := multi.EncodeName(ob, tc.name)
		if err != nil {
			t.Error(err)
			continue
		}
...
func ExampleEncodeName() {
	...
	mhbuf, _ := multi.EncodeName(buf, "sha1")
...
func TestTable(t *testing.T) {
	for k, v := range tCodes {
		if multi.Codes[k] != v {
			t.Error("Table mismatch: ", multi.Codes[k], v)
		}
		if multi.Names[v] != k {
			t.Error("Table mismatch: ", multi.Names[v], k)
		}
	}

	for name, code := range multi.Names {
		if tCodes[code] != name {
	...
	for code, name := range multi.Codes {
		if tCodes[code] != name {
			if strings.HasPrefix(name, "blake") {
				// skip these
				continue
...
func TestCast(t *testing.T) {
	for _, tc := range testCases {
		...
		if _, err := multi.Cast(nb); err != nil {
			t.Error(err)
			continue
		}

		if _, err = multi.Cast(ob); err == nil {
...
func TestDecodeErrorInvalid(t *testing.T) {
	_, err := multi.FromB58String("/ipfs/QmQTw94j68Dgakgtfd45bG3TZG6CAfc427UVRH4mugg4q4")
	if err != multi.ErrInvalidMultihash {
		t.Fatalf("expected: %s, got %s\n", multi.ErrInvalidMultihash, err)
	}
}

func TestBadVarint(t *testing.T) {
	_, err := multi.Cast([]byte{129, 128, 128, 128, 128, 128, 128, 128, 128, 128, 129, 1})
	if err != varint.ErrOverflow {
		t.Error("expected error from varint longer than 64bits, got: ", err)
	}
	_, err = multi.Cast([]byte{129, 128, 128})
	if err != varint.ErrUnderflow {
		t.Error("expected error from cut-off varint, got: ", err)
	}

	_, err = multi.Cast([]byte{129, 0})
	if err != varint.ErrNotMinimal {
		t.Error("expected error non-minimal varint, got: ", err)
	}

	_, err = multi.Cast([]byte{128, 0})
	if err != varint.ErrNotMinimal {
		t.Error("expected error non-minimal varint, got: ", err)
	}
}
