...
func (c *IdentitySignature) ToPlain() (*identityprovider.IdentitySignature, error) {
	...
	if err != nil {
		return nil, message.ErrIdentitySigDeserialization.Wrap(err)
	}
...
func (e *EntryV0) ToPlain(out iface.IPFSLogEntry, provider identityprovider.Interface, newClock func() iface.IPFSLogLamportClock) error {
	...
	if err != nil {
		return message.ErrKeyDeserialization.Wrap(err)
	}
...
func (c *Identity) ToPlain(provider identityprovider.Interface) (*identityprovider.Identity, error) {
	publicKey, err := hex.DecodeString(c.PublicKey)
	if err != nil {
		return nil, message.ErrIdentityDeserialization.Wrap(err)
	}
...
func (c *Entry) ToPlain(out iface.IPFSLogEntry, provider identityprovider.Interface, newClock func() iface.IPFSLogLamportClock) error {
	...
	if err != nil {
		return message.ErrSigDeserialization.Wrap(err)
	}

	clock := newClock()
	if err := c.Clock.ToPlain(clock); err != nil {
		return message.ErrClockDeserialization.Wrap(err)
	}
