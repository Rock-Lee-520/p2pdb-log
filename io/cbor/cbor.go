...
import (
	...
	"github.com/Rock-liyi/p2pdb-log/enc"
	...
	"github.com/Rock-liyi/p2pdb-log/io/jsonable"
...
func (i *IOCbor) DecodeRawJSONLog(node format.Node) (*iface.JSONLog, error) {
	...
	if err != nil {
		return nil, message.ErrCBOROperationFailed.Wrap(err)
	}
...
func (i *IOCbor) DecodeRawEntry(node format.Node, hash cid.Cid, p identityprovider.Interface) (iface.IPFSLogEntry, error) {
	...
	if err != nil {
		return nil, message.ErrDecrypt.Wrap(err)
	}
	...
	if err := obj.ToPlain(e, p, i.refClock.New); err != nil {
		return nil, message.ErrEntryDeserializationFailed.Wrap(err)
	}
		...
		atlas.BuildEntry(ic.Secp256k1PublicKey{}).
			...
			TransformMarshal(atlas.MakeMarshalTransformFunc(
				func(x ic.Secp256k1PublicKey) (string, error) {
					keyBytes, err := x.Raw()
					if err != nil {
						return "", message.ErrNotSecp256k1PubKey.Wrap(err)
					}
			...
			TransformUnmarshal(atlas.MakeUnmarshalTransformFunc(
				func(x string) (ic.Secp256k1PublicKey, error) {
					...
					if err != nil {
						return ic.Secp256k1PublicKey{}, message.ErrNotSecp256k1PubKey.Wrap(err)
					}
					...
					if !ok {
						return ic.Secp256k1PublicKey{}, message.ErrNotSecp256k1PubKey
					}
...
func (i *IOCbor) Write(ctx context.Context, ipfs core_iface.CoreAPI, obj interface{}, opts *iface.WriteOpts) (cid.Cid, error) {
	...
	if err != nil {
		return cid.Undef, message.ErrCBOROperationFailed.Wrap(err)
	}
	...
	if err != nil {
		return cid.Undef, message.ErrIPFSOperationFailed.Wrap(err)
	}
...
func (i *IOCbor) PreSign(entry iface.IPFSLogEntry) (iface.IPFSLogEntry, error) {
	...
	if err != nil {
		return nil, message.ErrEncrypt.Wrap(fmt.Errorf("unable to cbor entry: %w", err))
	}

	nonce, err := i.linkKey.DeriveNonce(NonceRefForEntry(entry))
	if err != nil {
		return nil, message.ErrEncrypt.Wrap(err)
	}

	encryptedLinks, err := i.linkKey.SealWithNonce(cborPayload, nonce)
	if err != nil {
		return nil, message.ErrEncrypt.Wrap(fmt.Errorf("unable to encrypt message"))
	}
...
func (i *IOCbor) DecryptLinks(entry *jsonable.EntryV2) (*jsonable.EntryV2, error) {
	...
	if err := i.cborUnmarshaller.Unmarshal(dec, links); err != nil {
		return nil, message.ErrEncrypt.Wrap(fmt.Errorf("unable to unmarshal decrypted message"))
	}
