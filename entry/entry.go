...
func CreateEntryWithIO(ctx context.Context, ipfsInstance core_iface.CoreAPI, identity *identityprovider.Identity, data iface.IPFSLogEntry, opts *iface.CreateEntryOptions, io iface.IO) (iface.IPFSLogEntry, error) {
	if ipfsInstance == nil {
		return nil, message.ErrIPFSNotDefined
	}

	if identity == nil {
		return nil, message.ErrIdentityNotDefined
	}

	if data == nil || !data.Defined() {
		return nil, message.ErrPayloadNotDefined
	}

	if data.GetLogID() == "" {
		return nil, message.ErrLogIDNotDefined
	}
	...
	if err != nil {
		return nil, message.ErrEntryNotHashable.Wrap(err)
	}
	...
	if err != nil {
		return nil, message.ErrIPFSOperationFailed.Wrap(err)
	}
...
func cidB58(c cid.Cid) (string, error) {
	...
	if err != nil {
		return "", message.ErrMultibaseOperationFailed.Wrap(err)
	}
...
func toBuffer(e *iface.Hashable) ([]byte, error) {
	if e == nil {
		return nil, message.ErrEntryNotDefined
	}
	...
	if err != nil {
		return nil, message.ErrJSONSerializationFailed.Wrap(err)
	}
...
func ToHashable(e iface.IPFSLogEntry) (*iface.Hashable, error) {
	...
	for i, n := range e.GetNext() {
		c, err := cidB58(n)
		if err != nil {
			return nil, message.ErrCIDSerializationFailed.Wrap(err)
		}
...
func (e *Entry) Verify(identity identityprovider.Interface, io iface.IO) error {
	if e == nil || !e.Defined() {
		return message.ErrEntryNotDefined
	}

	if len(e.Key) == 0 {
		return message.ErrKeyNotDefined
	}

	if len(e.Sig) == 0 {
		return message.ErrSigNotDefined
	}
	...
	if err != nil {
		return message.ErrEntryNotHashable.Wrap(err)
	}
	...
	if err != nil {
		return message.ErrInvalidPubKeyFormat.Wrap(err)
	}
...
func ToMultihashWithIO(ctx context.Context, e iface.IPFSLogEntry, ipfsInstance core_iface.CoreAPI, opts *iface.CreateEntryOptions, io iface.IO) (cid.Cid, error) {
	...
	if e == nil || !e.Defined() {
		return cid.Undef, message.ErrEntryNotDefined
	}

	if ipfsInstance == nil {
		return cid.Undef, message.ErrIPFSNotDefined
	}
...
func FromMultihashWithIO(ctx context.Context, ipfs core_iface.CoreAPI, hash cid.Cid, provider identityprovider.Interface, io iface.IO) (iface.IPFSLogEntry, error) {
	...
	if err != nil {
		return nil, message.ErrIPFSReadFailed.Wrap(err)
	}

	decoded, err := io.DecodeRawEntry(result, hash, provider)
	if err != nil {
		return nil, message.ErrIPFSReadUnmarshalFailed.Wrap(err)
	}
