...
import (
	...
	"github.com/Rock-liyi/p2pdb-log/accesscontroller"
	"github.com/Rock-liyi/p2pdb-log/entry"
...
func (l *IPFSLog) traverse(rootEntries iface.IPFSLogOrderedEntries, amount int, endHash string) (iface.IPFSLogOrderedEntries, error) {
	...
	if rootEntries == nil {
		return nil, message.ErrEntriesNotDefined
	}
...
func (l *IPFSLog) Iterator(options *IteratorOptions, output chan<- iface.IPFSLogEntry) error {
	amount := -1
	if options == nil {
		return message.ErrIteratorOptionsNotDefined
	}

	if output == nil {
		return message.ErrOutputChannelNotDefined
	}
	...
	if options.LTE != nil {
		...
		for _, c := range options.LTE {
			...
			if !ok {
				l.lock.RUnlock()
				return message.ErrFilterLTENotFound
			}
		...
		for _, c := range options.LT {
			...
			if !ok {
				l.lock.RUnlock()
				return message.ErrFilterLTNotFound
			}
	...
	l.lock.RUnlock()
	if err != nil {
		return message.ErrLogTraverseFailed.Wrap(err)
	}
...
func (l *IPFSLog) Join(otherLog iface.IPFSLog, size int) (iface.IPFSLog, error) {
	...
	if otherLog == nil || l == nil {
		return nil, message.ErrLogJoinNotDefined
	}
	...
	for _, k := range newItems.Keys() {
		go func(k string) {
			...
			if inErr := e.Verify(l.Identity.Provider, l.IO()); inErr != nil {
				err = message.ErrSigNotVerified.Wrap(inErr)
				return
			}
		}(k)
	}
	...
	wg.Wait()
	if err != nil {
		return nil, message.ErrLogJoinFailed.Wrap(err)
	}
...
func NewFromMultihash(ctx context.Context, services core_iface.CoreAPI, identity *identityprovider.Identity, hash cid.Cid, logOptions *LogOptions, fetchOptions *FetchOptions) (*IPFSLog, error) {
	...
	if logOptions == nil {
		return nil, message.ErrLogOptionsNotDefined
	}

	if fetchOptions == nil {
		return nil, message.ErrFetchOptionsNotDefined
	}
	...
	if err != nil {
		return nil, message.ErrLogFromMultiHash.Wrap(err)
	}
...
func NewFromEntryHash(ctx context.Context, services core_iface.CoreAPI, identity *identityprovider.Identity, hash cid.Cid, logOptions *LogOptions, fetchOptions *FetchOptions) (*IPFSLog, error) {
	...
	if err != nil {
		return nil, message.ErrLogFromEntryHash.Wrap(err)
	}
...
func NewFromJSON(ctx context.Context, services core_iface.CoreAPI, identity *identityprovider.Identity, jsonLog *iface.JSONLog, logOptions *LogOptions, fetchOptions *entry.FetchOptions) (*IPFSLog, error) {
	...
	if err != nil {
		return nil, message.ErrLogFromJSON.Wrap(err)
	}
...
func NewFromEntry(ctx context.Context, services core_iface.CoreAPI, identity *identityprovider.Identity, sourceEntries []iface.IPFSLogEntry, logOptions *LogOptions, fetchOptions *entry.FetchOptions) (*IPFSLog, error) {
	...
	if err != nil {
		return nil, message.ErrLogFromEntry.Wrap(err)
	}
