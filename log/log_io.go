package log // import "github.com/Rock-liyi/p2pdb-log"
...
import (
	...
	// "github.com/Rock-liyi/p2pdb-log/io"
)
...
func toMultihash(ctx context.Context, services core_iface.CoreAPI, log *IPFSLog) (cid.Cid, error) {
	if log.heads.Len() == 0 {
		return cid.Undef, message.ErrEmptyLogSerialization
	}
...
func fromJSON(ctx context.Context, services core_iface.CoreAPI, jsonLog *iface.JSONLog, options *iface.FetchOptions) (*Snapshot, error) {
	...
	if options.IO == nil {
		return nil, message.ErrLogOptionsNotDefined.Wrap(fmt.Errorf("missing IO field in fetch options"))
	}
